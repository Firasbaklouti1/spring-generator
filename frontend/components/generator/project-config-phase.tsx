...
/**
 * Render the project configuration UI and manage previewing, generating, and downloading projects.
 *
 * This component displays stack-specific configuration controls, dependency selection, and a generation summary.
 * It fetches dependency groups for the selected stack, builds payloads from the current project state, and
 * handles preview generation, ZIP downloads (from backend or from generated preview files), and a fallback
 * mock-spec download when backend generation fails. UI state and project settings are read from and written
 * to the generator store; user-visible notifications are emitted via toasts.
 *
